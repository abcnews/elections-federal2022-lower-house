...
const Article = () => {
  const [_graphicProps, setGraphicProps] = useState<GraphicProps>({});
      ...
      <div className="grid">
        {STATE_IDS.map(stateID => (
          <div className="state" key={stateID} id={stateID}>
            ...
            <MarkerText text={`#lhliveSTATE${stateID.toLowerCase()}`}></MarkerText>
          </div>
        ))}
      </div>
    </article>
...
whenDOMReady.then(() => render(<Article />, selectMounts('lhalllive')[0]));
